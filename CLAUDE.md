...
- B2 分五批：B2-1…B2-4／**B2-5a 已批核**（2026-07-25，HEAD `8674c07`，DEK 兩層鎖 live＋V-08 R-based recovery 限 v3＋v2 零回歸，crypto/shamir 基線零改動，test 123/123）→ **B2-5b（已開放，既有 vault 遷移＋全記錄加密＋in-memory 搜尋，release-gate 收尾，限 08-08）**。逐批 QA 驗收＋總監批核先開下一批。
...
- **B2 收尾前遞延項**：v3 UI 整合任務（shamir_screen UI＋seam 1 transfer 密碼＋seam 3 backup UI，Alpha 前）；B2-5a-native（V-05 auth-bound native，實體機 bucket）；實體 ARM API24 Argon2id 量測（release gate，實體機 bucket）。release gate 完整解除＝全記錄加密＋實體量測二者齊備。
