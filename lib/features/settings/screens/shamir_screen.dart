  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(
      ...
      bottom: TabBar(
        ...
        unselectedLabelColor: SanctumTheme.textTertiary,
...
class _GenerateTabState extends State<_GenerateTab> {
  ...
  void _snack(String m) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(m), backgroundColor: SanctumTheme.bg3, behavior: SnackBarBehavior.floating, duration: const Duration(seconds: 2))); }
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextFormField(
        ...
        decoration: InputDecoration(
          hintText: '輸入您的主密碼', filled: true, fillColor: SanctumTheme.bg2,
          ...
          suffixIcon: IconButton(icon: Icon(_showPw ? Icons.visibility_off : Icons.visibility, size: 18, color: SanctumTheme.textTertiary), onPressed: () => setState(() => _showPw = !_showPw)),
        ),
      ),
      const SizedBox(height: 24),
      ...
      SizedBox(width: double.infinity, child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: SanctumTheme.gold, foregroundColor: SanctumTheme.bg, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
        child: _generating ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: SanctumTheme.bg)) : const Text('生成碎片', style: TextStyle(fontWeight: FontWeight.w600)),
      )),
    ]),
  );

  Widget _results() => Column(children: [
    Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text('已生成 $_n 份碎片（需 $_k 份還原）', style: const TextStyle(fontSize: 13, color: SanctumTheme.textPrimary))),
    ...
    SafeArea(child: Padding(
      ...
      child: SizedBox(width: double.infinity, child: OutlinedButton(
        onPressed: _reset,
        style: OutlinedButton.styleFrom(foregroundColor: SanctumTheme.textTertiary, side: const BorderSide(color: SanctumTheme.border), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      for (int i = 0; i < _shareCount; i++) ...[
        ...
        TextFormField(
          controller: _ctrls[i],
          style: const TextStyle(color: SanctumTheme.textPrimary, fontSize: 12, fontFamily: 'monospace'),
          maxLines: 2,
          decoration: InputDecoration(
      ...
      if (_recovered != null) ...[
        ...
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(color: SanctumTheme.bg2, borderRadius: BorderRadius.circular(10), border: Border.all(color: SanctumTheme.gold.withValues(alpha: 0.4))),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('✅  已成功還原主密碼', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: SanctumTheme.gold)),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: Text(_showResult ? _recovered! : '•' * _recovered!.length.clamp(8, 20), style: const TextStyle(fontSize: 14, color: SanctumTheme.textPrimary, fontFamily: 'monospace'))),
              IconButton(icon: Icon(_showResult ? Icons.visibility_off : Icons.visibility, size: 18, color: SanctumTheme.textTertiary), onPressed: () => setState(() => _showResult = !_showResult)),
            ...
            const SizedBox(height: 8),
            const Text('⚠️  複製後請立即使用，不要截圖或儲存。', style: TextStyle(fontSize: 11, color: SanctumTheme.textTertiary)),
          ]),
        ),
      ],
...
class _ShareCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SelectableText(code, style: const TextStyle(fontSize: 11, color: SanctumTheme.textPrimary, fontFamily: 'monospace', letterSpacing: 1.0)),
    ]),
  );
}
...
class _MiniBtn extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => GestureDetector(onTap: onTap, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: SanctumTheme.bg3, borderRadius: BorderRadius.circular(6), border: Border.all(color: SanctumTheme.border)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 12, color: SanctumTheme.textTertiary), const SizedBox(width: 4), Text(label, style: const TextStyle(fontSize: 11, color: SanctumTheme.textTertiary))])));
}

class _InfoBox extends StatelessWidget {
  final String text; const _InfoBox(this.text);
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: SanctumTheme.bg2, borderRadius: BorderRadius.circular(8), border: Border.all(color: SanctumTheme.border)), child: Text(text, style: const TextStyle(fontSize: 12, color: SanctumTheme.textTertiary, height: 1.5)));
}

class _Label extends StatelessWidget {
  final String text; const _Label(this.text);
  @override
  Widget build(BuildContext context) => Text(text, style: const TextStyle(fontSize: 12, color: SanctumTheme.textTertiary));
}
...
class _SliderRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    SizedBox(width: 120, child: Text(label, style: const TextStyle(fontSize: 12, color: SanctumTheme.textTertiary))),
    Expanded(child: SliderTheme(data: SliderTheme.of(context).copyWith(activeTrackColor: SanctumTheme.gold, inactiveTrackColor: SanctumTheme.border, thumbColor: SanctumTheme.gold, overlayColor: SanctumTheme.gold.withValues(alpha: 0.12), trackHeight: 2),
      child: Slider(value: value.toDouble(), min: min.toDouble(), max: max.toDouble(), divisions: max - min, onChanged: (v) => onChanged(v.round())))),
    SizedBox(width: 28, child: Text('$value', textAlign: TextAlign.center, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: SanctumTheme.textPrimary))),
  ]);
}